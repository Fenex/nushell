...
impl ColorFrame {
    ...
    fn has_successful_child_frames(&self) -> bool {
        self.any_child_frame(|frame| !frame.error.is_some())
    }
    ...
    fn has_any_shape_descendents(&self) -> bool {
        if self.children.len() == 0 {
            ...
            for child in &self.children {
                match child {
                    ...
                    FrameChild::Shape(_) => return true,
                    FrameChild::Frame(frame) => {
                        if frame.has_any_shape_descendents() {
                            return true;
                        }
                    }
                }
